			...
			switchMap((user) => {
				return this._chatInfo$.pipe(
					...
					map((chatInfo) => {
						...
						const displayInfo =
							displayInfoOf === this._translocoService.translate("CHAT.SUPPLIER")
								? chatInfo.offer?.contact
								: chatInfo.buyer;
						const author =
							displayInfo?.fullName ||
							displayInfo?.personName ||
							`${this._translocoService.translate("CHAT.SUPPLIER")} of ${chatInfo.offer?.title}`;
						...
						messages.map((message: { author: any; }) => {
							const displayInfoOf =
								chat.seller?._id === message?.author
									...
									: this._translocoService.translate("CHAT.BUYER");
							const displayInfo =
								displayInfoOf === this._translocoService.translate("CHAT.SUPPLIER") ? chat.offer?.contact : chat.buyer;
