...
export class ClientOfferChatComponent implements OnInit, OnDestroy {
			...
			switchMap((user) =>
				this._chatInfo$.pipe(
					...
					map((chatInfo) => {
						...
						const displayInfo =
							role === this._translocoService.translate("CHAT.SELLER") ? chatInfo.offer?.contact : chatInfo.buyer;

						const author = displayInfo?.fullName || displayInfo?.personName || `${role} of ${chatInfo.offer?.title}`;
	...
	public subscribeOnChatInfo() {
		return this._socket.on("chat_info", (chatInfo) => {
			...
			this.displayContactsOpened = chatInfo.offer.openedFor.includes(chatInfo.buyer?._id);
			...
			if (user._id !== chatInfo.seller?._id && user?._id !== chatInfo.buyer?._id) {
