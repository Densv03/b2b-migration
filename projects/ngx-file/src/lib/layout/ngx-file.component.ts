...
export class B2bNgxFileComponent implements ControlValueAccessor, OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes["errors"]) {
      this.formControl.setErrors(changes['error'].currentValue);
    }
  }
  ...
  private findTotalSize(files: unknown[]): number {
    // @ts-ignore
    return <number>files.reduce((accum, curr) => accum += curr.size, 0)
  }
